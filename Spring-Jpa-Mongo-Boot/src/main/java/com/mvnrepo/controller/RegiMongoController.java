...
@Controller
@Profile({"prod"})
public class RegiMongoController {

	@Autowired
	IRegisterMongoService service;
	...
	public   String     listOfEmployees(ModelMap   modelMap) {
		List<EmployeeMongo>  getList = service.getListOfEmployees();
	...
	public   String      getEditEmployeePage(@RequestParam("email")String  id,ModelMap   modelMap) {
		Optional<EmployeeMongo>   opt = service.editEmployeePage(id);
		EmployeeMongo    employee=opt.get();
		modelMap.addAttribute("e",employee);
		return   "getEditEmployee";
	}
	
	@PostMapping("/updateEmployee")
	public    String    updateEmployee(@ModelAttribute("e")EmployeeMongo  employee, ModelMap  modelMap) {
		List<EmployeeMongo>  empList = service.getUpdateEmployee(employee);
	...
	public    String    deleteEmployee(@RequestParam("email")String  id, ModelMap   modelMap) {
		...
		List<EmployeeMongo>  empList = service.getDeleteEmployee(id);
		modelMap.addAttribute("employees", empList);
		return   "getListOfEmployees";
